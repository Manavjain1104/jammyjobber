...
from utils.semadb_utils import search_ids_metadata
...
DATASET_NOISY = "evaluation/dataset_noisy_150.csv"

def calculate_recall(label, query, model, labelname):
    embedding = process_data(query, model)
    ids = search_ids_metadata(EVALUATION_COLLECTION_QA, embedding)
    correct_ids = 0
    from pathlib import Path
    from csv import reader
    with open(Path(DATASET_NOISY), "r") as csv_file:
