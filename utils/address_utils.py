...
def is_postcode(address):
    return re.fullmatch(r'[A-Za-z][A-Za-z0-9]{1,3}\s*[0-9][A-Za-z]{2}', address) is not None
...
def is_postcode_in_region(postcode, region):
    ...
    if postcode_response.status_code == 200:
        return any(region.lower() in str(val).lower() for val in postcode_response.json()['result'].values())
    else:
        raise Exception(f"Error: {postcode_response.status_code}, {postcode_response.json()}")
