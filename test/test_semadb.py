...
class SemaDBTestCase(unittest.TestCase):
    def test_url_generation(self):
        assert semadb.collection_url(
            test_collection) == "https://semadb.p.rapidapi.com/collections/test_collection"
        assert semadb.points_url(
            test_collection) == "https://semadb.p.rapidapi.com/collections/test_collection/points"
        assert semadb.search_url(
            test_collection) == "https://semadb.p.rapidapi.com/collections/test_collection/points/search"
        assert semadb.points_url(
            "") == "https://semadb.p.rapidapi.com/collections//points"

    def test_points_generation(self):
        vector = [1.0, 1.0]
        externalId = 1
        assert semadb.new_point(vector, externalId) == {
            "vector":  vector, "metadata": {"externalId": externalId}}
    ...
    def test_add_points_ok(self, mock_post):
        ...
        response = semadb.add_points(
            test_collection, [semadb.new_point([1, 1], 1)])
        assert_is_not_none(response)
    ...
    def test_bulk_add_calls_add_points(self):
        ...
        with mock.patch(mock_function, return_value=response):
            assert semadb.bulk_add_points(test_collection,
                                          new_points_vectors,
                                          new_points_ids) == response
